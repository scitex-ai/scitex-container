"""`scitex-container skills` — list / get / install agent-facing skills.

Self-contained. No scitex-dev runtime dep — walks the package's own
`_skills/scitex-container/` directory directly.
"""
...
PKG = "scitex-container"


def _skills_root() -> Path:
    """Resolve the bundled `_skills/scitex-container/` directory."""
    import scitex_container

    pkg_dir = Path(scitex_container.__file__).parent
    return pkg_dir / "_skills" / PKG


def _list_skill_files(root: Path) -> list[Path]:
    """All `.md` files under `_skills/scitex-container/` (recursive), excluding SKILL.md."""
    if not root.is_dir():
        return []
...
def skills_group(ctx) -> None:
    """Agent-facing skills bundled with scitex-container.

    \b
    Examples:
      $ scitex-container skills list
      $ scitex-container skills get 01_installation
      $ scitex-container skills install                  # → ~/.scitex/dev/skills/scitex-container/
      $ scitex-container skills install --claude-symlink # also expose to ~/.claude/skills/scitex/
    """
...
def skills_list(as_json: bool) -> None:
    ...
    Example:
      $ scitex-container skills list
      $ scitex-container skills list --json
    """
    root = _skills_root()
...
def skills_get(name: str, as_json: bool) -> None:
    ...
    Example:
      $ scitex-container skills get 01_installation
      $ scitex-container skills get 02_quick-start --json
    """
    root = _skills_root()
...
@click.option(
    ...
    type=click.Path(),
    default=None,
    help="Destination dir (default: ~/.scitex/dev/skills/scitex-container/).",
)
@click.option(
    "--no-link",
    "no_link",
...
def skills_install(
    ...
    \b
    Default: symlink the entire `_skills/scitex-container/` dir to
    ~/.scitex/dev/skills/scitex-container/ so add/rename/delete in
    source propagates immediately.

    \b
    Example:
      $ scitex-container skills install
      $ scitex-container skills install --claude-symlink
      $ scitex-container skills install --no-link --dest /tmp/scitex-container-skills
    """
