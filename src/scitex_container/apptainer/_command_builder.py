...
def build_instance_start_script(
    ...
    # Extract flags between "exec" and container_path, filtering out
    # --pwd (valid for exec but not instance start).
    flags = exec_args[2:-1]
    # Remove --pwd and its argument
    i = flags.index("--pwd") if "--pwd" in flags else -1
    if i >= 0:
...
set -e
# Clean up stale instance from previous allocation (zombie protection)
apptainer instance list 2>/dev/null | grep -q {instance_quoted} && \
  {{ apptainer instance stop {instance_quoted} 2>/dev/null || true; sleep 1; }}
# Stop instance on job cancellation (SIGTERM from scancel)
trap 'apptainer instance stop {instance_quoted} 2>/dev/null; exit 0' TERM INT
apptainer instance start {flags_str} {container_quoted} {instance_quoted}
if [ $? -ne 0 ]; then echo "INSTANCE_FAILED"; exit 1; fi
echo "INSTANCE_READY"
# Keep allocation alive while instance is running
while apptainer instance list 2>/dev/null | grep -q {instance_quoted}; do sleep 10; done
